...
const UserProfile = (props) => {
    ...
    const validateForm = async () => {
        ...
        if (isAnyPasswordNotEmpty) {
            ...
            if (!currentPassword.trim()) {
                ...
                isValid = false;
            } else if (!(await props.passwordCheck(currentPassword.trim()))) {
    ...
    const HandleSubmit = async (e) => {
        ...
        if (validateForm()) {
            ...
            try {
                ...
                // Update user details
                if (sendDeleteResume) {
                    await props.deleteResumes(sendDeleteResume);
                }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setdeleteResume(userdata.Resume);
    ...
    const [sendDeleteResume, setsendDeleteResume] = useState([]);
    ...
    const handleResumeUpload = (e) => {
        ...
        if (file) {
            const reader = new FileReader();
            reader.onload = () => {
                // Upload the resume and update the local state
                setSendResume((prevResumes) => [...prevResumes, { name: file.name, data: reader.result.split(',')[1] }]);
                setResume((prevResumes) => [...prevResumes, file.name]);
            };
            reader.readAsDataURL(file);
        }

    };
    const handleRemoveResume = (attachment) => {
        // Remove the selected resume from the local state
        if (sendResume) {
            setSendResume((prevResumes) =>
                prevResumes.filter((resume) => resume.name !== attachment)
            );
        }

        setResume((prevResumes) => prevResumes.filter((resume) => resume !== attachment));
        // Check if attachment exists in deleteResume array and add it to sendDeleteResume array
        if (deleteResume.includes(attachment)) {
            setsendDeleteResume((prevDeleteResumes) => [...prevDeleteResumes, attachment]);
        }
    };
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-xl-12">
                                        <div className="dashboard-box">
                                            <div className="headline">
                                                <h3><i className="icon-material-outline-face"></i> My Resume</h3>
                                            </div>
                                                            ...
                                                            <div className="">
                                                                <div className="submit-field">
                                                                    {Resume && (
                                                                        <>
                                                                            <h5>Attachments</h5>
                                                                            <div className="attachments-container margin-top-0 margin-bottom-0 flex-nowrap">
                                                                                {Resume && Resume.map((attachment, index) => (
                                                                                    <div className="attachment-box ripple-effect" key={index}>
                                                                                        <div className="attachment-info">
                                                                                            <span>{attachment.split('.').slice(0, -1).join('.')}</span>
                                                                                            <i>{attachment.split('.').pop().toUpperCase()}</i>
                                                                                        </div>
                                                                                        {deleteResume.includes(attachment) && (
                                                                                            <button
                                                                                                type='button'
                                                                                                onClick={() => navigate(`/resume/${attachment}`)}
                                                                                        ...
                                                                                        <button
                                                                                            ...
                                                                                            onClick={() => handleRemoveResume(attachment)}
                                                                                        ></button>
                                                                                    </div>
                                                                                ))}
                                                                            </div>
                                                                    ...
                                                                    {Resume.length === 0 && (
                                                                        <h2 className='my-3'>No Resumes Found</h2>
                                                                    )}
                                                                        ...
                                                                        {Resume.length < 5 ? (
                                                                            ...
                                                                            <>
                                                                                <button
                                                                                    ...
                                                                                    type="button"
                                                                                    id='upload'
                                                                                    onClick={() => { enqueueSnackbar('Only Five Resumes can be Uploaded!!', { variant: 'warning' }) }}
                                                                                ></button>
                                                                                <label className="uploadButton-button ripple-effect" htmlFor="upload">
                                                                                    Upload Resume
                                                                                </label>
